...
def onnx_to_keras(onnx_model, input_names,
                  input_shapes=None, name_policy=None, verbose=True, change_ordering=False, test_c_order = False, dynamic_input = False):
    """
    ...
    for i, input_name in enumerate(input_names):
        for onnx_i in onnx_inputs:
            if onnx_i.name == input_name:
                ...
                print("-"*20)
                print(input_shape)
                if test_c_order == True:
                    input_shape[0], input_shape[2] = input_shape[2], input_shape[0]
