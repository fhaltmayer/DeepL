...
def moments(
  ...
  with ops.name_scope(name, "moments", [x, axes]):
    ...
    #tflite broken with math.pow inside squared difference
    sqdf = (y - array_ops.stop_gradient(mean)) * (y - array_ops.stop_gradient(mean))
    variance = math_ops.reduce_mean(
        sqdf, #math_ops.squared_difference(y, array_ops.stop_gradient(mean)),
        axes,
        keepdims=True,
        name="variance")
    if not keep_dims:
